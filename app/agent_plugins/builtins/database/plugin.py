...
class CreateProviderPresetInput(BaseModel):
    ...
    slug: str = Field(
        ..., description="Unique machine-readable identifier (e.g. openai)"
    )
    base_url: str = Field(..., description="Base API URL")
    auth_type: str = Field(
        ..., description="Authentication type: bearer, api_key, or none"
    )
    auth_config_key: str | None = Field(
        None, description="The key name for the secret reference (e.g. OPENAI_API_KEY)"
    )
    category: str | None = Field(
        "Cloud API", description="Category: Cloud API, Local Hosted, etc."
    )
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                new_preset = ProviderPreset(
                    ...
                    auth_config=(
                        {"secret_ref_id": input_data.auth_config_key}
                        if input_data.auth_config_key
                        else {}
                    ),
    ...
    async def update_provider_preset(self, args: dict) -> str:
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                if input_data.name:
                    preset.name = input_data.name
                if input_data.base_url:
