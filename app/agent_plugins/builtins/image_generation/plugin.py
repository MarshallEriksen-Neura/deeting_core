...
class ImageGenerationPlugin(AgentPlugin):
    ...
    async def handle_generate_image(
        ...
        if not __context__:
            return (
                "Error: Internal system error (Context missing for image generation)."
            )
        ...
        try:
            ...
            if not candidates:
                return (
                    "Error: No image generation providers are configured or available."
                )
            ...
            return f"Error selecting image provider: {e!s}"
            ...
            async with create_async_http_client(timeout=120.0) as client:
                response = await provider_instance.execute(
                    request_payload=extra_context[
                        "input"
                    ],  # This is 'request_payload' in execute signature
                    client=client,
