...
def _ensure_enabled() -> None:
    if not settings.LINUXDO_OAUTH_ENABLED:
        raise LinuxDoOAuthError(
            "LinuxDo OAuth 尚未启用", status.HTTP_503_SERVICE_UNAVAILABLE
        )
...
async def _exchange_code(client: httpx.AsyncClient, code: str) -> LinuxDoToken:
    ...
    try:
        ...
        raise LinuxDoOAuthError(
            f"LinuxDo token 接口请求失败: {exc}", status.HTTP_502_BAD_GATEWAY
        )
    ...
    try:
        ...
        raise LinuxDoOAuthError(
            "LinuxDo token 接口返回非 JSON 数据", status.HTTP_502_BAD_GATEWAY
        ) from exc

    access_token = payload.get("access_token")
    if not isinstance(access_token, str):
        raise LinuxDoOAuthError(
            "LinuxDo token 接口缺少 access_token", status.HTTP_502_BAD_GATEWAY
        )
    ...
    return LinuxDoToken(
        access_token=access_token, token_type=str(token_type), expires_in=expires
    )
...
async def _fetch_profile(
    ...
    try:
        ...
        raise LinuxDoOAuthError(
            f"LinuxDo 用户信息接口请求失败: {exc}", status.HTTP_502_BAD_GATEWAY
        )
    ...
    try:
        ...
        raise LinuxDoOAuthError(
            "LinuxDo 用户信息接口返回非 JSON 数据", status.HTTP_502_BAD_GATEWAY
        ) from exc
