    ...
    async def ingest_deep_dive(
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise Exception(f"Failed to communicate with Scout: {e!s}")
        ...
        logger.info(
            f"Scout returned {len(artifacts_data)} artifacts. Ingesting to Review Buffer..."
        )
    ...
    async def _save_to_review_buffer(
        ...
        if existing:
            ...
            if existing.content_hash == content_hash and existing.status == "indexed":
                logger.info(
                    f"Content for {url} unchanged and already indexed. Skipping."
                )
                return existing

            # Update existing to pending_review
            artifact = await self.repo.update(
                existing,
                {
