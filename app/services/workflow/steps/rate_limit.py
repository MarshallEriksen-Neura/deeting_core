...
class RateLimitExceededError(Exception):
    ...
    def __init__(
        self, limit_type: str, limit: int, retry_after: int = 60, scope: str = "unknown"
    ):
...
class RateLimitStep(BaseStep):
    ...
    def _build_rate_limit_specs(self, ctx: "WorkflowContext") -> list[dict[str, Any]]:
                ...
                specs.append(
                    {
                        ...
                        "tpm": ak_tpm,
                        "tpm_key": (
                            CacheKeys.rate_limit_tpm(f"ak:{ctx.api_key_id}", "chat")
                            if ak_tpm
                            else None
                        ),
                    }
        ...
        if ctx.client_ip:
            # 假设默认 IP 限制
            ip_rpm = (
                settings.RATE_LIMIT_IP_DEFAULT_RPM
            )  # 需在 settings 增加此配置，暂定 600
    ...
    def _select_strict_tpm_spec(
        self, specs: list[dict], limit_config: dict, ctx: "WorkflowContext"
    ) -> dict | None:
        ...
        try:
            ...
            if res[0] == 0:
                ...
                if len(res) >= 4:
                    ...
                    rejected_spec = (
                        specs[rejected_idx]
                        if 0 <= rejected_idx < len(specs)
                        else specs[0]
                    )
                    limit = res[2]
                    retry_after = res[3]
                    raise RateLimitExceededError(
                        "rpm", limit, retry_after, scope=rejected_spec["scope"]
                    )
                if len(res) >= 3:
            ...
            # 通过
            return {"remaining": res[1], "reset": res[2]}
    ...
    async def _check_tpm(self, spec: dict) -> float | None:
        ...
        try:
            ...
            if script_sha:
                ...
                if res[0] == 0:
                    raise RateLimitExceededError(
                        "tpm", tpm_limit, int(res[2]), scope=spec["scope"]
                    )
                return float(res[1])
            else:
                # Python Fallback
                ...
                last_update = (
                    float(bucket.get(b"last_update", now_sec)) if bucket else now_sec
                )
                ...
                if tokens < cost:
                    ...
                    await redis_client.hset(
                        key, mapping={"tokens": tokens, "last_update": now_sec}
                    )
                    await redis_client.expire(key, window + 1)
                    raise RateLimitExceededError(
                        "tpm", tpm_limit, retry_after, scope=spec["scope"]
                    )
