...
class SpecAgentDetectorStep(BaseStep):
        ...
        try:
            ...
            try:
                ...
                if confidence < 0.7:
                    ...
                    # 置信度太低,移除标记但不添加建议
                    cleaned_content = SPEC_AGENT_SUGGESTION_PATTERN.sub(
                        "", content
                    ).strip()
                    self._update_response_content(ctx, response, cleaned_content)
                    return StepResult(
                        status=StepStatus.SUCCESS, message="low_confidence"
                    )
    ...
    def _update_response_content(
        self, ctx: "WorkflowContext", response: dict, new_content: str
    ) -> None:
        """更新响应中的内容"""
