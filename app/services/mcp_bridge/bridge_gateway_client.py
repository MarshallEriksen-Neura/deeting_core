"""
Bridge Gateway (Tunnel Gateway) 内部客户端。

仅供内部网关调用云端 MCP 通道，不暴露给外部。
"""
...
class BridgeGatewayClient:
    """
    与云端 Tunnel Gateway 通信的轻量客户端。
    - 使用内部 token（X-Internal-Token）鉴权
    - 提供 agent 列表、工具列表、invoke/cancel、SSE 事件流
    """
