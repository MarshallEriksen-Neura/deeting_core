...
class ApiKey(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    key_prefix: Mapped[str] = mapped_column(
        String(12), nullable=False, comment="Key 前缀 (sk-ext- / sk-int-)"
    )
    key_hash: Mapped[str] = mapped_column(
        String(64), nullable=False, unique=True, comment="HMAC-SHA256 哈希"
    )
    key_hint: Mapped[str] = mapped_column(
        String(8), nullable=False, comment="Key 末 4 位 (****abcd)"
    )
    secret_hash: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="HMAC 签名专用 Secret 哈希 (独立于 key_hash)"
    )
    secret_hint: Mapped[str | None] = mapped_column(
        String(8), nullable=True, comment="Secret 末 4 位 (****wxyz)"
    )
    ...
    name: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Key 名称/描述"
    )
    ...
    tenant_id: Mapped[uuid.UUID | None] = mapped_column(
        SA_UUID(as_uuid=True), nullable=True, comment="外部 Key 绑定的租户 ID"
    )
    ...
    revoked_reason: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="吊销原因"
    )
...
class ApiKeyScope(Base, UUIDPrimaryKeyMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "api_key_id", "scope_type", "scope_value", name="uq_api_key_scope"
        ),
    )
...
class ApiKeyRateLimit(Base, UUIDPrimaryKeyMixin):
    ...
    tpm: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="每分钟 Token 数限制"
    )

    # 每日限制
    rpd: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="每日请求数限制"
    )
    tpd: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="每日 Token 数限制"
    )

    # 并发与突发
    concurrent_limit: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="并发请求数限制"
    )
    burst_limit: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="突发上限"
    )

    # 白名单
    is_whitelist: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="是否白名单 (跳过全局限流)"
    )
...
class ApiKeyQuota(Base, UUIDPrimaryKeyMixin):
    ...
    total_quota: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="总配额"
    )
    used_quota: Mapped[int] = mapped_column(
        BigInteger, nullable=False, default=0, comment="已用配额"
    )
    ...
    reset_period: Mapped[QuotaResetPeriod] = mapped_column(
        ...
        nullable=False,
        default=QuotaResetPeriod.MONTHLY,
        comment="重置周期",
    )
    reset_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="下次重置时间"
    )
...
class ApiKeyIpWhitelist(Base, UUIDPrimaryKeyMixin):
    ...
    ip_pattern: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="IP 或 CIDR (192.168.1.0/24)"
    )
    description: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="描述"
    )
...
class ApiKeyUsage(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "api_key_id", "stat_date", "stat_hour", name="uq_api_key_usage"
        ),
    ...
    stat_hour: Mapped[int] = mapped_column(
        SmallInteger, nullable=False, comment="统计小时 (0-23)"
    )

    # 统计值
    request_count: Mapped[int] = mapped_column(
        BigInteger, nullable=False, default=0, comment="请求数"
    )
    token_count: Mapped[int] = mapped_column(
        BigInteger, nullable=False, default=0, comment="Token 消耗"
    )
    cost: Mapped[Decimal] = mapped_column(
        Numeric(18, 8), nullable=False, default=Decimal("0"), comment="费用"
    )
    error_count: Mapped[int] = mapped_column(
        BigInteger, nullable=False, default=0, comment="错误数"
    )
