...
class KnowledgeArtifact(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    source_url: Mapped[str] = mapped_column(
        String(1024), nullable=False, comment="来源 URL"
    )
    title: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="网页标题"
    )

    raw_content: Mapped[str] = mapped_column(
        Text, nullable=False, comment="原始爬取内容 (Markdown)"
    )
    content_hash: Mapped[str] = mapped_column(
        String(64), nullable=False, comment="内容哈希，用于检测变更"
    )
...
class KnowledgeChunk(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (Index("ix_knowledge_chunk_artifact_id", "artifact_id"),)
    ...
    chunk_index: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="在原文中的顺序索引"
    )

    text_content: Mapped[str] = mapped_column(
        Text, nullable=False, comment="清洗后的切片文本"
    )
    ...
    embedding_id: Mapped[uuid.UUID | None] = mapped_column(
        SA_UUID(as_uuid=True), nullable=True, comment="对应向量库 (Qdrant) 中的 ID"
    )

    # 关系映射
    artifact: Mapped[KnowledgeArtifact] = relationship(
        "KnowledgeArtifact", back_populates="chunks"
    )

    def __repr__(self) -> str:
        return (
            f"<KnowledgeChunk(artifact={self.artifact_id}, index={self.chunk_index})>"
        )
