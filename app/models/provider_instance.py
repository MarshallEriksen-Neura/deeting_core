...
class ProviderInstance(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    user_id: Mapped[uuid.UUID | None] = mapped_column(
        ...
        nullable=True,
        index=True,
        comment="所属用户/工作区，为空表示平台公共实例",
    )
    preset_slug: Mapped[str] = mapped_column(
        String(80), nullable=False, index=True, comment="引用 provider_preset.slug"
    )
    name: Mapped[str] = mapped_column(
        String(80), nullable=False, comment="实例名称，用户自定义"
    )
    description: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="实例描述"
    )
    base_url: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="实例基础 URL，可覆盖模板"
    )
    icon: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="覆盖模板的图标引用"
    )
    credentials_ref: Mapped[str] = mapped_column(
        String(128), nullable=False, comment="密钥引用 ID（db:<uuid> 或别名）"
    )
    priority: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0", comment="路由优先级"
    )
...
class ProviderModel(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    instance_id: Mapped[uuid.UUID] = mapped_column(
        SA_UUID(as_uuid=True),
        ForeignKey("provider_instance.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    weight: Mapped[int] = mapped_column(
        Integer, nullable=False, default=100, server_default="100", comment="负载权重"
    )
    priority: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0", comment="优先级"
    )
    ...
    instance: Mapped["ProviderInstance"] = relationship(
        "ProviderInstance", back_populates="models"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "model_id",
            "upstream_path",
            name="uq_provider_model_identity",
        ),
        Index(
...
class ProviderCredential(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    secret_ref_id: Mapped[str] = mapped_column(
        String(128), nullable=False, comment="密钥引用 ID（db:<uuid>）"
    )
    weight: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0", comment="候选权重偏移"
    )
    priority: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0", comment="候选优先级偏移"
    )
    is_active: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, server_default="true", comment="是否启用"
    )
    ...
    instance: Mapped["ProviderInstance"] = relationship(
        "ProviderInstance", back_populates="credentials"
    )
