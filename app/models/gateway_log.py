...
class GatewayLog(Base, UUIDPrimaryKeyMixin):
    ...
    preset_id: Mapped[uuid.UUID | None] = mapped_column(
        SA_UUID(as_uuid=True), index=True, comment="命中的预设 ID"
    )

    model: Mapped[str] = mapped_column(
        String(128), nullable=False, comment="请求的上游模型名称"
    )
    status_code: Mapped[int] = mapped_column(
        Integer, nullable=False, index=True, comment="响应状态码"
    )

    duration_ms: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="总耗时(ms)"
    )
    ttft_ms: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="首包时间(ms)"
    )

    # 上游信息
    upstream_url: Mapped[str | None] = mapped_column(
        String(512), nullable=True, comment="上游地址"
    )
    ...
    cost_upstream: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0, server_default="0.0", comment="上游成本"
    )
    cost_user: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0, server_default="0.0", comment="用户扣费"
    )
    ...
    error_code: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="统一错误码"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<GatewayLog(id={self.id}, model={self.model}, status={self.status_code})>"
        )
