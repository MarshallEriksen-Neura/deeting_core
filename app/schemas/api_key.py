...
class ApiKeyScopeBase(BaseModel):
    ...
    permission: ScopePermissionEnum = Field(
        ScopePermissionEnum.ALLOW, description="权限类型"
    )
...
class ApiKeyQuotaBase(BaseModel):
    ...
    reset_period: QuotaResetPeriodEnum = Field(
        QuotaResetPeriodEnum.MONTHLY, description="重置周期"
    )
