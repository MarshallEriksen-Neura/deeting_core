...
class SpecNodeBase(BaseModel):
    ...
    desc: str | None = Field(None, description="节点描述，用于 UI 展示")
...
class ActionNode(SpecNodeBase):
    ...
    instruction: str = Field(
        ...,
        description="给 Sub-Agent 的具体自然语言指令 (e.g. 'Search for RTX 4090 price')",
    )

    # 辅助字段
    required_tools: list[str] = Field(
        default_factory=list,
        description="建议使用的工具列表 (e.g. ['mcp.search.tavily'])",
    )

    # 兼容旧逻辑 (可选)
    worker: str = Field("generic", description="执行者身份 (默认为通用 generic)")
    args: dict[str, Any] = Field(
        default_factory=dict, description="结构化参数 (如果 Planner 确定了参数)"
    )

    output_as: str | None = Field(None, description="输出变量名，供后续节点引用")
...
class LogicRule(BaseModel):
    ...
    condition: str = Field(
        ..., description="判断条件表达式 (e.g., '$.has_32G_stock == true')"
    )
    next_node: str = Field(..., description="条件满足时跳转的节点 ID")
    desc: str | None = Field(None, description="规则描述")
...
class ReplanTriggerNode(SpecNodeBase):
    ...
    new_goal: str | None = Field(None, description="重规划的新目标 (可选)")
