...
class SkillRegistryBase(BaseSchema):
    ...
    runtime: str | None = Field(
        None, max_length=40, description="运行时类型（如 opensandbox）"
    )
    ...
    source_revision: str | None = Field(
        None, max_length=128, description="源码版本/提交"
    )
    ...
    manifest_json: dict[str, Any] = Field(
        default_factory=dict, description="技能清单/Manifest"
    )
    env_requirements: dict[str, Any] = Field(
        default_factory=dict, description="运行环境依赖"
    )
...
class SkillRegistryCreate(SkillRegistryBase):
    id: str = Field(
        ..., max_length=120, description="技能唯一标识（如 core.tools.crawler）"
    )
