...
class AssetUploadInitRequest(BaseSchema):
    content_hash: str = Field(
        ..., description="SHA-256 内容哈希（hex）", min_length=64, max_length=64
    )
    ...
    expires_seconds: int | None = Field(
        None, description="上传预签名有效期（秒）", gt=0
    )
...
class AssetUploadInitResponse(BaseSchema):
    ...
    asset_url: str | None = Field(
        None, description="可直接展示的访问 URL（命中去重时返回）"
    )
    upload_url: str | None = Field(
        None, description="预签名上传 URL（未命中去重时返回）"
    )
    upload_headers: dict[str, str] | None = Field(
        None, description="上传时需携带的 Header"
    )
