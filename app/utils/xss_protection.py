...
def is_safe_attr_value(attr_name: str, attr_value: str) -> bool:
    ...
    if attr_name == "style":
        return not re.search(
            r"expression|javascript:|url\s*\(\s*javascript:", attr_value, re.IGNORECASE
        )
...
def validate_and_sanitize_user_input(
    data: str | dict[str, Any] | list[Any], max_length: int = 10000
) -> str | dict[str, Any] | list[Any]:
    """
    ...
    if isinstance(data, str):
        ...
        for key, value in data.items():
            sanitized_key = (
                validate_and_sanitize_user_input(str(key), max_length)
                if isinstance(key, str)
                else key
            )
            sanitized_dict[sanitized_key] = validate_and_sanitize_user_input(
                value, max_length
            )
        return sanitized_dict
