...
def build_embedding_body(
    model: str, text: str, input_type: str | None
) -> dict[str, Any]:
...
async def ensure_collection(
    client: httpx.AsyncClient, base_url: str, name: str, dim: int, api_key: str | None
) -> None:
    ...
    body = {
        ...
        },  # force HNSW building immediately to avoid plain mode edge cases
    }
    create = await client.put(
        f"{base_url}/collections/{name}", headers=headers, json=body
    )
...
async def upsert_point(
    ...
    body = {
        "points": [
            {"id": pid, "vector": {QDRANT_VECTOR_NAME: vector}, "payload": payload}
        ]
    }
...
async def search_point(
    ...
    if search_type:
        body.setdefault("params", {})["hnsw_ef"] = int(
            os.getenv("QDRANT_HNSW_EF", "128")
        )
        body["search_type"] = search_type

    resp = await client.post(
        f"{base_url}/collections/{name}/points/search", headers=headers, json=body
    )
    try:
        resp.raise_for_status()
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    embed_key = args.api_key or env_first(
        "TEST_EMBED_API_KEY", "EMBED_API_KEY", "TEST_API_KEY"
    )
    model = args.model or env_first(
        "TEST_EMBED_MODEL", "EMBED_MODEL", fallback="text-embedding-3-small"
    )
    input_type = args.input_type or env_first(
        "TEST_EMBED_INPUT_TYPE", "EMBED_INPUT_TYPE", "INPUT_TYPE"
    )
    qdrant_url = args.qdrant_url or env_first("TEST_QDRANT_URL", "QDRANT_URL")
    qdrant_key = args.qdrant_api_key or env_first(
        "TEST_QDRANT_API_KEY", "QDRANT_API_KEY"
    )
    collection = args.collection or os.getenv("QDRANT_COLLECTION") or "test_embed"

    if not embed_url or not embed_key:
        print(
            "missing embedding url/api-key; set TEST_EMBED_URL & TEST_EMBED_API_KEY or pass --url/--api-key"
        )
        return 1
    if not qdrant_url:
        print(
            "missing qdrant url; set TEST_QDRANT_URL or QDRANT_URL or pass --qdrant-url"
        )
        return 1
    ...
    async with httpx.AsyncClient() as client:
        vector = await fetch_embedding(
            client, embed_url, embed_key, model, args.text, input_type, args.timeout
        )
        print(
            f"[embed] dim={len(vector)} preview={[round(vector[i],4) for i in range(0, min(4,len(vector)))]}"
        )
        ...
        pid = await upsert_point(
            ...
            collection,
            vector,
            {"source": "test_qdrant_roundtrip", "text": args.text},
            qdrant_key,
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Embedding -> Qdrant roundtrip tester."
    )
    parser.add_argument(
        "--url", help="Embedding endpoint (env TEST_EMBED_URL/EMBED_API_URL)"
    )
    parser.add_argument(
        "--api-key", help="Embedding API key (env TEST_EMBED_API_KEY/EMBED_API_KEY)"
    )
    parser.add_argument("--model", help="Embedding model name")
    parser.add_argument(
        "--input-type",
        help="input_type for asymmetric models (e.g. search_document/query)",
    )
    ...
    parser.add_argument(
        "--qdrant-url", help="Qdrant base URL, e.g. http://localhost:6333"
    )
    parser.add_argument("--qdrant-api-key", help="Qdrant API key if required")
    parser.add_argument(
        "--collection", help="Qdrant collection name (default test_embed)"
    )
