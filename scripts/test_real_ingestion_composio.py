...
class RealTestLLMService:
    def __init__(self):
        ...
        logger.info(f"✅ RealTestLLMService initialized with model={self.model}, base_url={base_url}")
...
async def main():
    ...
    with patch("app.services.skill_registry.manifest_generator.llm_service", real_llm):
        ...
        async with AsyncSessionLocal() as session:
            ...
            try:
                dry_run_result = await dry_run_service.run(SKILL_ID, allow_self_heal=False)
                ...
                if skill.status == "active":
                    ...
                    logger.error(f">>> Failure Reason from DB: {metrics.get('last_error')}")
