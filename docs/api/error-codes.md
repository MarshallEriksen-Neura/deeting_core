...
| 错误码 | 说明 | 解决方案 |
|--------|------|----------|
| `CODE_MODE_DIRECT_TOOL_BLOCKED` | 在 Code Mode 可用时，模型直接调用了未放行工具（默认放行 `search_sdk/execute_code_plan/consult_expert_network/search_knowledge`） | 优先走 `search_sdk -> execute_code_plan`；或通过 `CODE_MODE_DIRECT_TOOL_ALLOWLIST` 增加直调放行工具 |
