...
## 8. 里程碑（建议）
- **M0**：设计落地 + Manifest v1 + 表结构原型  
- **M1**：爬虫秘书落库 + Qdrant 检索  
- **M2**：OpenSandbox 端到端执行验证（docx 示例）
...
**对策**：引入 **Dry Run 预演**：入库前由 OpenSandbox 执行 `example_code`。  
未通过者禁止进入 `active`，进入 `needs_review`。  
...
### 13.2 Dry Run 预演
- 对所有新入库技能执行 Dry Run，失败即进入 `dry_run_fail`。  
- 失败后自动重试一次，并记录错误日志。  
