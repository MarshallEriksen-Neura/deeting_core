...
2.  **Hydrate**: 后端拿着 IDs 进行 **单次批量回查**（Assistant/Version/Review/RoutingState），获取最新版本的 `system_prompt` 与 **关联工具集 (Tool Definitions)**。
...
> 降级策略：当 Qdrant 未配置或检索失败时，回退到系统默认专家（若默认专家不存在则返回空候选）。
...
*   **Summary 解耦**: 会话摘要仅用于压缩历史上下文，**不承担人格身份**。专家切换时直接替换 System Prompt，summary 仍保留并置于 system 前置位置。
