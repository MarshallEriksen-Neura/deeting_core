...
async def test_billing_calculation():
    ...
    ctx.set(
        "routing",
        "pricing_config",
        {"input_per_1k": 0.03, "output_per_1k": 0.06, "currency": "USD"},
    )
...
async def test_billing_internal_channel_no_deduct():
    ...
    with patch.object(
        BillingStep, "_deduct_balance", AsyncMock()
    ) as mock_deduct, patch.object(BillingStep, "_record_usage", AsyncMock()):
...
async def test_billing_without_pricing_is_free():
    ...
    with patch.object(
        ...
        BillingStep, "_record_usage", AsyncMock()
    ) as mock_usage:
