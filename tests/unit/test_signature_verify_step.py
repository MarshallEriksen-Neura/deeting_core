...
from app.services.workflow.steps.signature_verify import (
    SignatureError,
    SignatureVerifyStep,
)
...
async def test_signature_requires_secret_when_configured():
    ...
    service = ApiKeyService(
        repository=AsyncMock(), redis_client=None, secret_key="jwt-secret"
    )
    ...
    with patch.object(
        ApiKeyService, "validate_key", AsyncMock(return_value=principal)
    ), patch.object(
        ApiKeyService, "_compute_key_hash", side_effect=service._compute_key_hash
    ), patch.object(
        ApiKeyService, "revoke_key", AsyncMock()
    ), patch.object(
        ApiKeyService, "check_ip", AsyncMock(return_value=True)
    ):

        # 未提供 secret 应失败
        with pytest.raises(SignatureError, match="API secret required"):
            await step._verify_signature(
                ctx, api_key, ts, nonce, "bad", api_secret=None
            )

        # 提供正确 secret 应通过
        signature = _sign(api_key, secret, ts, nonce)
        result = await step._verify_signature(
            ctx, api_key, ts, nonce, signature, api_secret=secret
        )
...
async def test_signature_continuous_failure_freeze(dummy_redis):
    ...
    with patch.object(
        ...
        for i in range(1, 5):
            with pytest.raises(
                SignatureError, match=f"Signature mismatch \\(failures={i}\\)"
            ):
        ...
        assert await dummy_redis.get("gw:blacklist:ak-fail") == "1"
...
async def test_signature_success_resets_counter(dummy_redis):
    ...
    with patch.object(
        ...
        # 2. 成功一次，应重置
        correct_sig = _sign(
            api_key, api_key, ts, "n2"
        )  # No secret, fallback to api_key
