...
async def test_check_rate_limit_allows(mock_cache):
    step = RateLimitStep()
    ctx = WorkflowContext(
        channel=Channel.EXTERNAL, tenant_id="test_tenant", api_key_id="test_key"
    )
    # Mock routing config
    ...
    mock_cache.evalsha.side_effect = [[1, 9, 60], [1, 100, 0]]  # RPM  # TPM
...
async def test_check_rate_limit_rpm_exceeded(mock_cache):
    step = RateLimitStep()
    ctx = WorkflowContext(channel=Channel.EXTERNAL, tenant_id="test_tenant")
    ctx.set("routing", "limit_config", {"rpm": 10})

    # RPM exceeded: {0, remaining, retry_after}
    mock_cache.evalsha.side_effect = [[0, 0, 30]]
...
async def test_check_rate_limit_tpm_exceeded(mock_cache):
    ...
    mock_cache.evalsha.side_effect = [[1, 9, 60], [0, 0, 15]]  # RPM  # TPM exceeded
