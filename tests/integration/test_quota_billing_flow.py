...
from app.models.billing import BillingTransaction, TransactionStatus
...
async def test_quota_billing_flow_with_sync(db_session):
    ...
    assert (
        abs(db_balance - expected_balance) < 0.000001
...
async def test_quota_cost_adjustment(db_session):
    ...
    assert (
        abs(redis_balance - expected_balance) < 0.000001
