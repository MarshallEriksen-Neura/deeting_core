...
async def test_billing_integration_insufficient_balance(monkeypatch):
    ...
    with patch(
        ...
        with patch(
            "app.services.workflow.steps.routing.RoutingStep.execute"
        ) as mock_routing:
            ...
            async def mock_routing_execute(ctx):
                ...
                ctx.set(
                    "routing",
                    "pricing_config",
                    {"input_per_1k": 0.03, "output_per_1k": 0, "currency": "USD"},
                )
            ...
            with patch(
                "app.services.workflow.steps.upstream_call.UpstreamCallStep.execute"
            ) as mock_upstream:
                ...
                async def mock_execute(ctx):
                    ...
                    ctx.upstream_result = MagicMock(
                        provider="openai", status_code=200, error_code=None
                    )
                    return StepResult(
                        status=StepStatus.SUCCESS, data={"response": mock_response}
                    )
