...
down_revision: str | None = "20260114_03_add_api_key_id_to_gateway_log"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "tenant_id", sa.UUID(), nullable=True, comment="租户 ID（为空表示全局）"
        ),
        sa.Column(
            "type",
            sa.String(length=40),
        ...
        sa.Column(
            "source", sa.String(length=120), nullable=True, comment="来源模块/服务"
        ),
        sa.Column(
            "dedupe_key", sa.String(length=120), nullable=True, comment="去重键（幂等）"
        ),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column(
            "read_at", sa.DateTime(timezone=True), nullable=True, comment="已读时间"
        ),
        sa.Column(
            "archived_at", sa.DateTime(timezone=True), nullable=True, comment="归档时间"
        ),
        sa.Column(
            "pinned_at", sa.DateTime(timezone=True), nullable=True, comment="置顶时间"
        ),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
    ...
    op.create_index(
        "ix_notification_receipt_notification_id",
        "notification_receipt",
        ["notification_id"],
    )
    op.create_index(
        "ix_notification_receipt_user_id", "notification_receipt", ["user_id"]
    )
    op.create_index(
        "ix_notification_receipt_tenant_id", "notification_receipt", ["tenant_id"]
    )
    op.create_index(
        "ix_notification_receipt_user_read",
        "notification_receipt",
        ["user_id", "read_at"],
    )
    op.create_index(
        "ix_notification_receipt_user_archived",
        "notification_receipt",
        ["user_id", "archived_at"],
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_notification_receipt_user_archived", table_name="notification_receipt"
    )
    op.drop_index(
        "ix_notification_receipt_user_read", table_name="notification_receipt"
    )
    op.drop_index(
        "ix_notification_receipt_tenant_id", table_name="notification_receipt"
    )
    op.drop_index("ix_notification_receipt_user_id", table_name="notification_receipt")
    op.drop_index(
        "ix_notification_receipt_notification_id", table_name="notification_receipt"
    )
    op.drop_index(
        "idx_notification_receipt_created_at", table_name="notification_receipt"
    )
