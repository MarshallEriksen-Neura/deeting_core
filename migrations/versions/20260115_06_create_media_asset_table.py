...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "size_bytes", sa.Integer(), nullable=False, comment="内容大小（字节）"
        ),
        sa.Column(
            "content_type", sa.String(length=120), nullable=False, comment="内容类型"
        ),
        sa.Column(
            "object_key", sa.String(length=512), nullable=False, comment="对象存储 Key"
        ),
        sa.Column(
            "etag", sa.String(length=128), nullable=True, comment="对象存储 ETag"
        ),
        sa.Column(
            "uploader_user_id",
        ...
        sa.UniqueConstraint(
            "content_hash", "size_bytes", name="uq_media_asset_hash_size"
        ),
