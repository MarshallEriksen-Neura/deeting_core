...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status", registration_status, nullable=False, server_default="scheduled"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_registration_windows_start_end"),
        "registration_windows",
        ["start_time", "end_time"],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "reserved_at", sa.DateTime(timezone=True), nullable=True, comment="预占时间"
        ),
        sa.Column(
            "used_at", sa.DateTime(timezone=True), nullable=True, comment="使用时间"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["window_id"], ["registration_windows.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(op.f("ix_invite_codes_code"), "invite_codes", ["code"], unique=True)
    op.create_index(
        op.f("ix_invite_codes_window_id"), "invite_codes", ["window_id"], unique=False
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_registration_windows_start_end"), table_name="registration_windows"
    )
