...
def upgrade() -> None:
    with op.batch_alter_table("bandit_arm_state") as batch:
        ...
        batch.add_column(
            sa.Column(
                "provider_model_id", postgresql.UUID(as_uuid=True), nullable=False
            )
        )
        batch.create_index(
            "ix_bandit_arm_state_provider_model_id", ["provider_model_id"], unique=True
        )
        batch.create_unique_constraint(
            "uq_bandit_arm_state_model", ["provider_model_id"]
        )
...
def downgrade() -> None:
    with op.batch_alter_table("bandit_arm_state") as batch:
        ...
        batch.add_column(
            sa.Column("preset_item_id", postgresql.UUID(as_uuid=True), nullable=False)
        )
        batch.create_index(
            "ix_bandit_arm_state_preset_item_id", ["preset_item_id"], unique=True
        )
