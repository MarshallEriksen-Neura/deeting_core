...
def upgrade() -> None:
    ...
    # 先创建枚举类型
    op.execute(
        "CREATE TYPE transaction_type AS ENUM ('deduct', 'recharge', 'refund', 'adjust')"
    )
    op.execute(
        "CREATE TYPE transaction_status AS ENUM ('pending', 'committed', 'reversed')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "daily_reset_at", sa.Date(), nullable=True, comment="日配额最后重置日期"
        ),
        # 月配额
        sa.Column(
            "monthly_quota",
        ...
        sa.Column(
            "monthly_reset_at", sa.Date(), nullable=True, comment="月配额最后重置日期"
        ),
        # 分钟级限流
        sa.Column(
            "rpm_limit",
    ...
    op.create_index(
        "ix_tenant_quota_tenant_active", "tenant_quota", ["tenant_id", "is_active"]
    )
    ...
    op.create_table(
        ...
        # 幂等键
        sa.Column(
            "trace_id", sa.String(64), nullable=False, comment="请求追踪 ID（幂等键）"
        ),
        # 交易信息
        sa.Column(
            "type",
        ...
        # 扣费前后余额
        sa.Column(
            "balance_before", sa.Numeric(18, 6), nullable=False, comment="扣费前余额"
        ),
        sa.Column(
            "balance_after", sa.Numeric(18, 6), nullable=False, comment="扣费后余额"
        ),
        # 备注
    ...
    op.create_index(
        "ix_billing_transaction_tenant_id", "billing_transaction", ["tenant_id"]
    )
    op.create_index(
        "ix_billing_transaction_api_key_id", "billing_transaction", ["api_key_id"]
    )
    op.create_index(
        "ix_billing_transaction_trace_id", "billing_transaction", ["trace_id"]
    )
    ...
    op.create_index(
        "ix_billing_tx_tenant_created",
        "billing_transaction",
        ["tenant_id", "created_at"],
    )
    op.create_index(
        "ix_billing_tx_status_created", "billing_transaction", ["status", "created_at"]
    )
